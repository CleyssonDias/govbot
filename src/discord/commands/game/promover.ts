...
createCommand({
    name: "promover",
    description: "🎇 Promover um usuario.",
    ...
    {
        name: "cargo",
        description: "🧡 Cargo para promover.",
        type: ApplicationCommandOptionType.String,
        required: true,
        autocomplete: true
    }],
    async autocomplete(interaction) {
        ...
        try {
            ...
            if (!rolesFound || !Array.isArray(rolesFound.promote)) return [];
            const choices = rolesFound.promote
                .filter(p => {
                    if (!focus) return true;
    ...
    async run(interaction) {
        ...
        rolesFound.promote.forEach(res => {
        ...
        if (!podePromover) {
            await message.edit(res.error(
                `### ${icon.error} Você não tem permissão para promover.`
            ));
            return;
        }
        ...
        if (user.length > 1) {
            for (const name of user) {
             ...
             const roleeaki = interaction.guild.roles.cache.find(role => role.name === cargo);

            await message.edit(res.default(
                `### ${icon.cheack} Os usuários ${user.join(', ')} foram promovidos para o cargo @${roleeaki?.id}.`
            ));
            return;
        }
        ...
        await message.edit(res.default(
            `### ${icon.cheack} O usuário ${user[0]} foi promovido para o cargo <@&${rolee?.id}>.`
        ));
    }
});